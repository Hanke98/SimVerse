...
namespace dyno
{
    ...
    __global__ void BatchCholeskySolveVarSizeKernel(
        DArray2D<Real> A_packed,
        const DevArr2D<Real> b_packed,
        DevArr2D<Real> x_packed,
        const DArray<int> n_list,
        int num_envs,
        DArray<int> skip_flag)
    {
    ...
    __global__ void BatchCholeskySolveVarSizeKernel(
        DevMat2D<Real> A_packed,
        const DevArr2D<Real> b_packed,
        DevArr2D<Real> x_packed,
        DArray<int> skip_flag)
    {
        const int env_id = blockIdx.x;
        if(env_id >= x_packed.NumBlocks())
            return;
        if(skip_flag.size() > 0)
            if(skip_flag[env_id])
                return;
        ...
        const int n = x_packed.Sizes()[env_id];
        ...
        for(int i = 0; i < n; i++)
        {
            ...
            {
                const int ij = i * lda + j;
                Real sum = A_packed.AtBlock(env_id, i, j);
                for(int k = 0; k < j; k++)
                {
                    const int ik = i * lda + k;
                    const int jk = j * lda + k;
                    sum -= A_packed.AtBlock(env_id, i, k) * A_packed.AtBlock(env_id, j, k);
                }
                ...
                if(i == j)
                {
                    if(sum < eps)
                        sum = eps;
                    A_packed.AtBlock(env_id, i, j) = sqrtf(sum);
                }
                else
                {
                    const int jj = j * lda + j;
                    const Real d = A_packed.AtBlock(env_id, j, j);
                    A_packed.AtBlock(env_id, i, j) = (fabsf(d) > eps) ? (sum / d) : 0.f;
                }
            }

            for(int j = i + 1; j < n; j++)
                A_packed.AtBlock(env_id, i, j) = 0.f;
        }
        ...
        for(int i = 0; i < n; i++)
        {
            Real sum = b_packed(env_id, i);
            for(int k = 0; k < i; k++)
                sum -= A_packed.AtBlock(env_id, i, k) * x_packed(env_id, k);

            const Real lii = A_packed.AtBlock(env_id, i, i);
            x_packed(env_id, i) = sum / lii;
        }
        ...
        for(int i = n - 1; i >= 0; i--)
        {
            Real sum = x_packed(env_id, i);
            for(int k = i + 1; k < n; k++)
                sum -= A_packed.AtBlock(env_id, k, i) * x_packed(env_id, k);
    ...
    template __global__ void BatchDenseMatrixVectorMul(DArray2D<Real> mat, DevArr2D<Real> vec, DevArr2D<Real> out, DArray<int> rows, DArray<int> cols, int num_sys, bool is_incremental, DArray<int> skip_flag);
    template __global__ void BatchDenseMatrixVectorMul(DArray2D<int> mat, DevArr2D<int> vec, DevArr2D<int> out, DArray<int> rows, DArray<int> cols, int num_sys, bool is_incremental, DArray<int> skip_flag);
