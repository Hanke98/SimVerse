...
namespace dyno
{
    ...
    __global__ void DofCountAndBuildIndexKernel(
        ...
        DArray2D<int> parent_idx_all, 
        DArray2D<int> is_static_all, 
    ...
    __global__ void InitQposKernel(DArray<int> batch_bodies, DevArr2D<Real> batch_qpos, 
        ...
        DArray2D<Real> joint_qpos, DArray2D<int> joint_qpos_offset,
        DArray2D<int> parent_idx, DArray2D<int> joint_type, DArray2D<int> is_static, int num_envs)
    {
        int env_id = blockIdx.x * blockDim.x + threadIdx.x;
        if(env_id >= num_envs)
            return;
